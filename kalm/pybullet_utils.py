# Refactored from pybullet-planning: https://github.com/caelan/pybullet-planning
...
def str_from_object(obj):  # str_object
    ...
    if type(obj) in [dict, defaultdict]:  # isinstance(obj, dict):
        return "{{{}}}".format(", ".join("{}: {}".format(*pair) for pair in sorted(tuple(map(str_from_object, pair)) for pair in obj.items())))
...
def cache_decorator(function):
    ...
    def get_cached(*args, **kwargs):
        ...
        # store the value
        if self._cache.force_immutable and hasattr(value, "flags") and len(value.shape) > 0:
...
def load_pybullet(filename, fixed_base=False, scale=1.0, **kwargs):
    ...
    with LockRenderer():
        ...
        if filename.endswith(".urdf"):
            body = p.loadURDF(filename, useFixedBase=fixed_base, flags=flags, globalScaling=scale, physicsClientId=CLIENT)
...
URDF_FLAGS = [p.URDF_USE_INERTIA_FROM_FILE, p.URDF_USE_SELF_COLLISION, p.URDF_USE_SELF_COLLISION_EXCLUDE_PARENT, p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS]
...
CameraInfo = namedtuple("CameraInfo", ["width", "height", "viewMatrix", "projectionMatrix", "cameraUp", "cameraForward", "horizontal", "vertical", "yaw", "pitch", "dist", "target"])
...
def get_camera_pose():
    ...
    view_matrix = compute_view_matrix(target_point, dist, yaw, pitch, roll=0.0, z_up=True)
    # get_ray_from_to
...
def get_projection_matrix(width, height, vertical_fov, near, far):
    ...
    projection_matrix = p.computeProjectionMatrixFOV(fov=fov_degrees, aspect=aspect, nearVal=near, farVal=far, physicsClientId=CLIENT)
...
def compute_view_matrix(target_position, distance, yaw, pitch, roll=0.0, z_up=True):
    view_matrix = p.computeViewMatrixFromYawPitchRoll(target_position, distance, math.degrees(yaw), math.degrees(pitch), math.degrees(roll), upAxisIndex=2 if z_up else 1, physicsClientId=CLIENT)
...
def compute_camera_pose(camera_point, target_point=np.zeros(3)):
    ...
    # camera_pose = multiply(target_pose, Pose(point=Point(x=-distance)))
    # print(camera_pose)
    view_matrix = compute_view_matrix(target_point, distance, yaw, pitch, roll=0.0, z_up=True)
    return pose_from_tform(view_matrix)


def get_image(camera_pos=None, target_pos=None, width=640, height=480, vertical_fov=60.0, near=0.02, far=5.0, tiny=False, segment=False, **kwargs):
    ...
    if (camera_pos is None) or (target_pos is None):
        ...
        view_matrix = p.computeViewMatrix(cameraEyePosition=camera_pos, cameraTargetPosition=target_pos, cameraUpVector=up_vector, physicsClientId=CLIENT)
...
def get_image_at_pose(camera_pose, camera_matrix, far=5.0, **kwargs):
    ...
    return get_image(camera_point, target_point, width=width, height=height, vertical_fov=vertical_fov, far=far, **kwargs)
...
def dump_joint(body, joint):
    print("Joint id: {} | Name: {} | Type: {} | Circular: {} | Lower: {:.3f} | Upper: {:.3f}".format(joint, get_joint_name(body, joint), JOINT_TYPES[get_joint_type(body, joint)], is_circular(body, joint), *get_joint_limits(body, joint)))
...
def dump_link(body, link):
    ...
    print("Link id: {} | Name: {} | Joint: {} | Parent: {} | Mass: {} | Collision: {} | Visual: {}".format(link, get_link_name(body, link), joint_name, get_link_name(body, get_link_parent(body, link)), get_mass(body, link), len(get_collision_data(body, link)), num_visual))
...
def dump_body(body, fixed=False, links=True):
    print("Body id: {} | Name: {} | Rigid: {} | Fixed: {}".format(body, get_body_name(body), is_rigid_body(body), is_fixed_base(body)))
    for joint in get_joints(body):
    ...
    print("Link id: {} | Name: {} | Mass: {} | Collision: {} | Visual: {}".format(base_link, get_base_name(body), get_mass(body), len(get_collision_data(body, base_link)), num_visual))
    for link in get_links(body):
        dump_link(body, link)
...
JointInfo = namedtuple("JointInfo", ["jointIndex", "jointName", "jointType", "qIndex", "uIndex", "flags", "jointDamping", "jointFriction", "jointLowerLimit", "jointUpperLimit", "jointMaxForce", "jointMaxVelocity", "linkName", "jointAxis", "parentFramePos", "parentFrameOrn", "parentIndex"])
...
def get_labeled_configuration(body):
    ...
    return dict(safe_zip(get_joint_names(body, movable_joints), get_joint_positions(body, movable_joints)))
...
LinkState = namedtuple("LinkState", ["linkWorldPosition", "linkWorldOrientation", "localInertialFramePosition", "localInertialFrameOrientation", "worldLinkFramePosition", "worldLinkFrameOrientation"])
...
DynamicsInfo = namedtuple("DynamicsInfo", ["mass", "lateral_friction", "local_inertia_diagonal", "local_inertial_pos", "local_inertial_orn", "restitution", "rolling_friction", "spinning_friction", "contact_damping", "contact_stiffness"])  # , 'body_type'])
...
LinkInfo = named_tuple("LinkInfo", *unzip([("mass", STATIC_MASS), ("collision_id", NULL_ID), ("visual_id", NULL_ID), ("point", unit_point()), ("quat", unit_quat()), ("inertial_point", unit_point()), ("inertial_quat", unit_quat()), ("parent", 0), ("joint_type", p.JOINT_FIXED), ("joint_axis", unit_point())]))


def create_body(collision_id=NULL_ID, visual_id=NULL_ID, mass=STATIC_MASS):
    return p.createMultiBody(baseMass=mass, baseCollisionShapeIndex=collision_id, baseVisualShapeIndex=visual_id, physicsClientId=CLIENT)
...
def create_faces(mesh, scale=1.0, mass=STATIC_MASS, collision=True, color=GREY, **kwargs):
...
VisualShapeData = namedtuple("VisualShapeData", ["objectUniqueId", "linkIndex", "visualGeometryType", "dimensions", "meshAssetFileName", "localVisualFrame_position", "localVisualFrame_orientation", "rgbaColor", "textureUniqueId"])
...
CollisionShapeData = namedtuple("CollisionShapeData", ["object_unique_id", "linkIndex", "geometry_type", "dimensions", "filename", "local_frame_pos", "local_frame_orn"])
...
def set_texture(body, texture=None, link=BASE_LINK, shape_index=NULL_ID):
    if texture is None:
        texture = NULL_ID
    return p.changeVisualShape(body, link, shapeIndex=shape_index, textureUniqueId=texture, physicsClientId=CLIENT)
...
def aabb_overlap(aabb1, aabb2):
    ...
    lower1, upper1 = aabb1
    lower2, upper2 = aabb2
    return all(l1 <= u2 for l1, u2 in zip(lower1, upper2)) and all(l2 <= u1 for l2, u1 in zip(lower2, upper1))
...
def get_closest_points(body1, body2, link1=None, link2=None, max_distance=MAX_DISTANCE, use_aabb=False):
    if use_aabb and not aabb_overlap(get_buffered_aabb(body1, link1, max_distance=max_distance / 2.0), get_buffered_aabb(body2, link2, max_distance=max_distance / 2.0)):
        return []
    ...
    if (link1 is None) and (link2 is None):
        ...
        results = p.getClosestPoints(bodyA=body1, bodyB=body2, linkIndexA=link1, distance=max_distance, physicsClientId=CLIENT)
    elif link1 is None:
        results = p.getClosestPoints(bodyA=body1, bodyB=body2, linkIndexB=link2, distance=max_distance, physicsClientId=CLIENT)
    else:
        results = p.getClosestPoints(bodyA=body1, bodyB=body2, linkIndexA=link1, linkIndexB=link2, distance=max_distance, physicsClientId=CLIENT)
...
def pairwise_collisions(body, obstacles, link=None, **kwargs):
    return any(pairwise_collision(body1=body, body2=other, link1=link, **kwargs) for other in obstacles if body != other)
...
RayResult = namedtuple("RayResult", ["objectUniqueId", "linkIndex", "hit_fraction", "hit_position", "hit_normal"])  # TODO: store Ray here
...
def get_difference_fn(body, joints):
    ...
    def fn(q2, q1):
        return tuple(circular_difference(value2, value1) if circular else (value2 - value1) for circular, value2, value1 in zip(circular_joints, q2, q1))
...
def get_self_link_pairs(body, joints, disabled_collisions=set(), only_moving=True):
    ...
    check_link_pairs = list(filter(lambda pair: (pair not in disabled_collisions) and (pair[::-1] not in disabled_collisions), check_link_pairs))
...
def get_collision_fn(body, joints, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(), custom_limits={}, use_aabb=False, cache=False, max_distance=MAX_DISTANCE, **kwargs):
    # TODO: convert most of these to keyword arguments
    check_link_pairs = get_self_link_pairs(body, joints, disabled_collisions) if self_collisions else []
    moving_links = frozenset(link for link in get_moving_links(body, joints) if can_collide(body, link))  # TODO: propagate elsewhere
    ...
    get_obstacle_aabb = cached_fn(get_buffered_aabb, cache=cache, max_distance=max_distance / 2.0, **kwargs)
    ...
    def collision_fn(q, verbose=False):
        ...
        get_moving_aabb = cached_fn(get_buffered_aabb, cache=True, max_distance=max_distance / 2.0, **kwargs)

        for link1, link2 in check_link_pairs:
            # Self-collisions should not have the max_distance parameter
            # TODO: self-collisions between body and attached_bodies (except for the link adjacent to the robot)
            if (not use_aabb or aabb_overlap(get_moving_aabb(body), get_moving_aabb(body))) and pairwise_link_collision(body, link1, body, link2):  # , **kwargs):
        ...
        for body1, body2 in product(moving_bodies, obstacles):
            if (not use_aabb or aabb_overlap(get_moving_aabb(body1), get_obstacle_aabb(body2))) and pairwise_collision(body1, body2, **kwargs):
                # print(get_body_name(body1), get_body_name(body2))
                if verbose:
                    print(body1, body2)
                return True
...
def interpolate_joint_waypoints(body, joints, waypoints, resolutions=None, collision_fn=lambda *args, **kwargs: False, **kwargs):
...
def plan_waypoints_joint_motion(body, joints, waypoints, start_conf=None, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(), resolutions=None, custom_limits={}, max_distance=MAX_DISTANCE, use_aabb=False, cache=True):
    if start_conf is None:
        start_conf = get_joint_positions(body, joints)
    assert len(start_conf) == len(joints)
    collision_fn = get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions, custom_limits=custom_limits, max_distance=max_distance, use_aabb=use_aabb, cache=cache)
...
def plan_joint_motion(body, joints, end_conf, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(), weights=None, resolutions=None, max_distance=MAX_DISTANCE, use_aabb=False, cache=True, custom_limits={}, algorithm=None, **kwargs):
    ...
    return solve(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, algorithm=algorithm, weights=weights, **kwargs)
...
def plan_lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn, **kwargs):
    ...
    path, samples, edges, colliding_vertices, colliding_edges = lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn, num_samples=200, **kwargs)
    if path is None:
        return path
...
def get_closest_angle_fn(body, joints, weights=None, reversible=True, linear_tol=0.0, **kwargs):
    assert len(joints) == 3
    weights = get_default_weights(body, joints, weights)
    ...
    def closest_fn(q1, q2):
        ...
        for direction in [0, PI] if reversible else [0]:  # TODO: previously was [PI]
            ...
            distance = angular_distance_fn(q1[2:], [angle]) + linear_distance + angular_distance_fn([angle], q2[2:])
...
def get_nonholonomic_extend_fn(body, joints, resolutions=None, angular_tol=0.0, **kwargs):
    assert len(joints) == 3
    resolutions = get_default_resolutions(body, joints, resolutions)
...
def get_dubins_extend_fn(body, joints, turning_radius=1e-3, step_size=0.01, **kwargs):  # meters  # meters
    assert len(joints) == 3
    import dubins
...
def plan_nonholonomic_motion(body, joints, end_conf, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(), weights=None, resolutions=None, reversible=True, linear_tol=EPSILON, angular_tol=0.0, max_distance=MAX_DISTANCE, use_aabb=False, cache=True, custom_limits={}, algorithm=None, **kwargs):

    assert len(joints) == len(end_conf) == 3
    sample_fn = get_sample_fn(body, joints, custom_limits=custom_limits)
    distance_fn = get_nonholonomic_distance_fn(body, joints, weights=weights, reversible=reversible, linear_tol=linear_tol)  # , angular_tol=angular_tol)
    extend_fn = get_nonholonomic_extend_fn(body, joints, resolutions=resolutions, reversible=reversible, linear_tol=linear_tol, angular_tol=angular_tol)
    ...
    if algorithm is None:
        ...
        return birrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)
    path = solve(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, algorithm=algorithm, **kwargs)  # weights=weights, # TODO: deliberately excluding for PRM unless circular
    # return path
...
def plan_base_joint_motion(robot, joints, goal_positions, attachments=[], obstacles=None, holonomic=False, reversible=False, **kwargs):
    ...
    if holonomic:
        return plan_joint_motion(robot, joints, goal_positions, attachments=attachments, obstacles=obstacles, **kwargs)
    ...
    return plan_nonholonomic_motion(robot, joints, goal_positions, reversible=reversible, linear_tol=1e-6, angular_tol=0.0, attachments=attachments, obstacles=obstacles, **kwargs)
...
def get_dynamical_limits(robot, joints, max_velocities=None, max_accelerations=None, duration_to_max=1.0):
    if max_velocities is None:
...
def get_acceleration_fn(robot, joints, max_velocities=None, max_accelerations=None, duration_to_max=1.0, **kwargs):
    from motion_planners.primitives import get_duration_fn

    max_velocities, max_accelerations = get_dynamical_limits(robot, joints, max_velocities, max_accelerations, duration_to_max=1.0)
...
def smooth_path(robot, joints, path, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(), resolutions=None, max_distance=MAX_DISTANCE, use_aabb=False, cache=True, custom_limits={}, max_velocities=None, max_accelerations=None, duration_to_max=1.0, **kwargs):
    if path is None:
        return None
    ...
    collision_fn = get_collision_fn(robot, joints, obstacles, attachments, self_collisions, disabled_collisions, custom_limits=custom_limits, use_aabb=use_aabb, cache=cache, max_distance=max_distance)
...
def plan_base_motion(body, end_conf, base_limits, obstacles=[], direct=False, weights=1 * np.ones(3), resolutions=0.05 * np.ones(3), max_distance=MAX_DISTANCE, algorithm=None, **kwargs):
    def sample_fn():
...
def is_placed_on_aabb(body, bottom_aabb, above_epsilon=1e-2, below_epsilon=0.0):
    ...
    return ((bottom_z_max - abs(below_epsilon)) <= top_z_min <= (bottom_z_max + abs(above_epsilon))) and (aabb_contains_aabb(aabb2d_from_aabb(top_aabb), aabb2d_from_aabb(bottom_aabb)))
...
def is_center_on_aabb(body, bottom_aabb, above_epsilon=1e-2, below_epsilon=0.0):
    ...
    bottom_z_max = bottom_aabb[1][2]
    return ((bottom_z_max - abs(below_epsilon)) <= top_z_min <= (bottom_z_max + abs(above_epsilon))) and (aabb_contains_point(base_center[:2], aabb2d_from_aabb(bottom_aabb)))
...
def get_constraints():
    ...
    """
    return [p.getConstraintUniqueId(i, physicsClientId=CLIENT) for i in range(p.getNumConstraints(physicsClientId=CLIENT))]
...
ConstraintInfo = namedtuple("ConstraintInfo", ["parentBodyUniqueId", "parentJointIndex", "childBodyUniqueId", "childLinkIndex", "constraintType", "jointAxis", "jointPivotInParent", "jointPivotInChild", "jointFrameOrientationParent", "jointFrameOrientationChild", "maxAppliedForce"])
...
def add_pose_constraint(body, pose=None, max_force=None):
    ...
    if pose is None:
        pose = get_pose(body)
    position, quat = pose
    constraint = p.createConstraint(body, link, -1, -1, jointType=p.JOINT_FIXED, jointAxis=[0, 0, 0], parentFramePosition=unit_point(), childFramePosition=position, parentFrameOrientation=unit_quat(), childFrameOrientation=quat, physicsClientId=CLIENT)
...
def add_fixed_constraint(body, robot, robot_link=BASE_LINK, max_force=None):
    ...
    constraint = p.createConstraint(robot, robot_link, body, body_link, p.JOINT_FIXED, jointAxis=unit_point(), parentFramePosition=point, childFramePosition=unit_point(), parentFrameOrientation=quat, childFrameOrientation=unit_quat(), physicsClientId=CLIENT)  # Both seem to work
    if max_force is not None:
        p.changeConstraint(constraint, maxForce=max_force, physicsClientId=CLIENT)
    return constraint
...
def remove_fixed_constraint(body, robot, robot_link):
    for constraint in get_fixed_constraints():
        constraint_info = get_constraint_info(constraint)
        if (body == constraint_info.childBodyUniqueId) and (BASE_LINK == constraint_info.childLinkIndex) and (robot == constraint_info.parentBodyUniqueId) and (robot_link == constraint_info.parentJointIndex):
...
def velocity_control_joint(body, joint, velocity=0.0, **kwargs):
    joint_kwargs = get_control_joint_kwargs(body, joint, **kwargs)
    return p.setJointMotorControl2(body, joint, p.VELOCITY_CONTROL, targetVelocity=velocity, physicsClientId=CLIENT, **joint_kwargs)  # Note that the targetVelocity is not the maximum joint velocity
...
def control_joints(body, joints, positions=None, velocities=None, position_gain=None, velocity_scale=None, max_force=None):
    ...
    if velocity_scale is not None:
        for i, joint in enumerate(joints):
            control_joint(body, joint, position=positions[i], velocity=velocities[i], position_gain=position_gain, velocity_scale=velocity_scale, max_force=max_force)
        return None
    ...
    return p.setJointMotorControlArray(bodyUniqueId=body, jointIndices=joints, controlMode=p.POSITION_CONTROL, targetPositions=positions, targetVelocities=velocities, physicsClientId=CLIENT, **kwargs)
...
def joint_controller_hold2(body, joints, positions, velocities=None, tolerance=1e-2 * np.pi, position_gain=0.05, velocity_gain=0.01):
    """
    Keeps other joints in place
    """
...
def compute_jacobian(robot, link, point=unit_point(), positions=None, velocities=None, accelerations=None):
    ...
    translate, rotate = p.calculateJacobian(robot, link, point, positions, velocities, accelerations, physicsClientId=CLIENT)
...
def inverse_kinematics_helper(robot, link, target_pose, null_space=None):
    ...
    if null_space is not None:
        ...
        kinematic_conf = p.calculateInverseKinematics(robot, link, target_point, lowerLimits=lower, upperLimits=upper, jointRanges=ranges, restPoses=rest, physicsClientId=CLIENT)
...
def interpolate_poses(pose1, pose2, pos_step_size=0.01, ori_step_size=np.pi / 16):
    pos1, quat1 = pose1
    pos2, quat2 = pose2
    num_steps = max(2, int(math.ceil(max(np.divide(get_pose_distance(pose1, pose2), [pos_step_size, ori_step_size])))))
    yield pose1
    for w in np.linspace(0, 1, num=num_steps, endpoint=True)[1:-1]:
...
def multiple_sub_inverse_kinematics(robot, first_joint, target_link, target_pose, max_attempts=1, max_solutions=INF, max_time=INF, custom_limits={}, first_close=True, **kwargs):
    ...
    for attempt in irange(max_attempts):
        ...
        sub_kinematic_conf = inverse_kinematics(sub_robot, sub_target_link, target_pose, max_time=max_time - elapsed_time(start_time), **kwargs)
...
def plan_cartesian_motion(robot, first_joint, target_link, waypoint_poses, max_iterations=200, max_time=INF, custom_limits={}, **kwargs):
...
def add_text(text, position=unit_point(), color=BLACK, lifetime=None, parent=NULL_ID, parent_link=BASE_LINK):
    return p.addUserDebugText(str(text), textPosition=position, textColorRGB=color[:3], lifeTime=get_lifetime(lifetime), parentObjectUniqueId=parent, parentLinkIndex=parent_link, physicsClientId=CLIENT)  # textSize=1,
...
def add_line(start, end, color=BLACK, width=1, lifetime=None, parent=NULL_ID, parent_link=BASE_LINK):
    ...
    return p.addUserDebugLine(start, end, lineColorRGB=color[:3], lineWidth=width, lifeTime=get_lifetime(lifetime), parentObjectUniqueId=parent, parentLinkIndex=parent_link, physicsClientId=CLIENT)
...
def draw_link_name(body, link=BASE_LINK, position=Point(y=0.2)):
    return add_text(get_link_name(body, link), position=position, parent=body, parent_link=link)
