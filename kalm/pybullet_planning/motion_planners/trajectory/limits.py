        ...
        for k in range(spline.c.shape[-1]):
            ...
            if v_max is not None:
                ...
                if any(abs(vel_poly(t)) > v_max[k] for t in boundary_ts):
                    return False
                if any(not isinstance(r, complex) and (t0 <= r <= t1) for s in signs for r in (vel_poly + s * np.poly1d([v_max[k]])).roots):
                    return False
            # a_max = None
            ...
            if a_max is not None:  # INF
                ...
                if any(not isinstance(r, complex) and (t0 <= r <= t1) for s in signs for r in (accel_poly + s * np.poly1d([a_max[k]])).roots):
                    return False
    return True
...
def check_spline(spline, v_max=None, a_max=None, verbose=False, **kwargs):
    ...
    if v_max is not None:
        ...
        if verbose:  # or violation:
            print("Violation: {} | Max velocity: {:.3f}/{:.3f} (at time {:.3f})".format(violation, v, get_max_velocity(v_max), t))
        if violation:
            return False
    return True
...
def minimize_objective(objective, lower, upper, num=100, max_time=INF, max_iterations=100, verbose=False, **kwargs):
    ...
    from scipy.optimize import (
        minimize,  # , line_search, brute, basinhopping, minimize_scalar
    )
    ...
    for iteration in range(num):
        ...
        if max_iterations is None:
            ...
            result = minimize(objective, x0=x0, bounds=bounds, **kwargs)  # maxiter=max_iterations,  # method=None, jac=None,
