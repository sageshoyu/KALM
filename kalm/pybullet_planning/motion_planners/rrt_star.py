...
def rrt_star(start, goal, distance_fn, sample_fn, extend_fn, collision_fn, radius, max_time=INF, max_iterations=INF, goal_probability=0.2, informed=True):
    """
    ...
    while (elapsed_time(start_time) < max_time) and (iteration < max_iterations):
        ...
        if iteration % PRINT_FREQUENCY == 0:
            ...
            print("Iteration: {} | Time: {:.3f} | Success: {} | {} | Cost: {:.3f}".format(iteration, elapsed_time(start_time), success, do_goal, cost))
        iteration += 1
        ...
        if len(path) == 0:
            continue
        new = OptimalNode(path[-1], parent=nearest, d=distance_fn(nearest.config, path[-1]), path=path[:-1], iteration=iteration)
        # if safe and do_goal:
