...
class DegreePRM(PRM):
    ...
    def grow(self, samples):
        ...
        for v1 in new_vertices:
            degree = 0
            for _, v2 in sorted(filter(lambda pair: (pair[1] != v1) and (pair[0] <= self.connect_distance), map(lambda v: (self.distance_fn(v1.q, v.q), v), self.vertices.values())), key=operator.itemgetter(0)):  # TODO - slow, use nearest neighbors
...
def prm(start, goal, distance_fn, sample_fn, extend_fn, collision_fn, target_degree=4, connect_distance=INF, num_samples=100):  # , max_time=INF):
    """
    ...
    if target_degree is None:
        roadmap = DistancePRM(distance_fn, extend_fn, collision_fn, samples=samples, connect_distance=connect_distance)
    else:
        roadmap = DegreePRM(distance_fn, extend_fn, collision_fn, samples=samples, target_degree=target_degree, connect_distance=connect_distance)
