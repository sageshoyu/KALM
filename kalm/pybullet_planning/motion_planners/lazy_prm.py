...
def wastar_search(start_v, end_v, neighbors_fn, cost_fn=unit_cost_fn, heuristic_fn=zero_heuristic_fn, max_cost=INF, max_time=INF, **kwargs):
...
class Roadmap(object):
    def __init__(self, extend_fn, weights=None, distance_fn=None, cost_fn=None, p_norm=2, max_degree=5, max_distance=INF, approximate_eps=0.0, **kwargs):
        # TODO: custom cost_fn
    ...
    def add_samples(self, samples):
        ...
        for v1, sample in self.nearest.add_data(samples):
            ...
            for d, v2, _ in self.nearest.query_neighbors(sample, k=max_degree, eps=self.approximate_eps, p=self.p_norm, distance_upper_bound=self.max_distance):
    ...
    def is_colliding(self, v1, v2):
        edge = (v1, v2)
        return self.colliding_vertices.get(v1, False) or self.colliding_vertices.get(v2, False) or self.colliding_edges.get(edge, False)
...
def lazy_prm(start, goal, sample_fn, extend_fn, collision_fn, distance_fn=None, cost_fn=None, roadmap=None, num_samples=100, weights=None, circular={}, p_norm=2, lazy=True, max_cost=INF, max_time=INF, w=1.0, meta=False, verbose=True, **kwargs):  # , max_paths=INF):
    """
    ...
    while (elapsed_time(start_time) < max_time) and (path is None):  # TODO: max_attempts
        lazy_path = wastar_search(start_vertex, end_vertex, neighbors_fn=roadmap.neighbors_fn, cost_fn=weight_fn, heuristic_fn=heuristic_fn, max_cost=max_cost, max_time=max_time - elapsed_time(start_time), w=w)
        if lazy_path is None:
            break
        if verbose:
            print("Candidate | Length: {} | Cost: {:.3f} | Vertices: {} | Samples: {} | Degree: {:.3f} | Time: {:.3f}".format(len(lazy_path), compute_path_cost(lazy_path, cost_fn=weight_fn), len(roadmap.colliding_vertices), len(roadmap.colliding_intermediates), degree, elapsed_time(start_time)))
    ...
    if path is None:
        ...
        if verbose:
            print("Failure | Forward: {} | Backward: {} | Vertices: {} | Samples: {} | Degree: {:.3f} | Time: {:.3f}".format(len(forward_visited), len(backward_visited), len(roadmap.colliding_vertices), len(roadmap.colliding_intermediates), degree, elapsed_time(start_time)))
        return Solution(path, samples, edges, roadmap.colliding_vertices, roadmap.colliding_edges)

    if verbose:
        print("Solution | Length: {} | Cost: {:.3f} | Vertices: {} | Samples: {} | Degree: {:.3f} | Time: {:.3f}".format(len(path), compute_path_cost(path, cost_fn=weight_fn), len(roadmap.colliding_vertices), len(roadmap.colliding_intermediates), degree, elapsed_time(start_time)))
...
def create_param_sequence(initial_samples=100, step_samples=100, **kwargs):
    ...
    return (merge_dicts(kwargs, {"num_samples": num_samples}) for num_samples in irange(start=initial_samples, stop=INF, step=step_samples))


def lazy_prm_star(start, goal, sample_fn, extend_fn, collision_fn, distance_fn=None, cost_fn=None, max_cost=INF, success_cost=INF, param_sequence=None, resuse=True, weights=None, circular={}, p_norm=2, max_time=INF, verbose=False, **kwargs):
    ...
    for i, params in enumerate(param_sequence):
        ...
        if remaining_time <= 0.0:
            break
        if verbose:
            print("\nIteration: {} | Cost: {:.3f} | Elapsed: {:.3f} | Remaining: {:.3f} | Params: {}".format(i, best_cost, elapsed_time(start_time), remaining_time, params))
        if (roadmap is None) or not resuse:
            roadmap = Roadmap(extend_fn, weights=weights, distance_fn=distance_fn, cost_fn=cost_fn, circular=circular)
            roadmap.add_samples([start, goal] + sample_until(sample_fn, params["num_samples"]))

        new_path = lazy_prm(start, goal, sample_fn, extend_fn, collision_fn, roadmap=roadmap, cost_fn=cost_fn, weights=weights, circular=circular, p_norm=p_norm, max_time=remaining_time, max_cost=best_cost, verbose=verbose, **params, **kwargs)[0]
