...
PR2_GROUPS = {
    ...
    arm_from_arm(LEFT_ARM): ["l_shoulder_pan_joint", "l_shoulder_lift_joint", "l_upper_arm_roll_joint", "l_elbow_flex_joint", "l_forearm_roll_joint", "l_wrist_flex_joint", "l_wrist_roll_joint"],
    arm_from_arm(RIGHT_ARM): ["r_shoulder_pan_joint", "r_shoulder_lift_joint", "r_upper_arm_roll_joint", "r_elbow_flex_joint", "r_forearm_roll_joint", "r_wrist_flex_joint", "r_wrist_roll_joint"],
    gripper_from_arm(LEFT_ARM): ["l_gripper_l_finger_joint", "l_gripper_r_finger_joint", "l_gripper_l_finger_tip_joint", "l_gripper_r_finger_tip_joint"],
    gripper_from_arm(RIGHT_ARM): ["r_gripper_l_finger_joint", "r_gripper_r_finger_joint", "r_gripper_l_finger_tip_joint", "r_gripper_r_finger_tip_joint"],
...
SIDE_HOLDING_LEFT_ARM = [0.39277395, 0.33330058, 0.0, -1.52238431, 2.72170996, -1.21946936, -2.98914779]
REST_LEFT_ARM = [2.13539289, 1.29629967, 3.74999698, -0.15000005, 10000.0, -0.10000004, 10000.0]
...
COMPACT_LEFT_ARM = [PI / 4, 0.0, PI / 2, -5 * PI / 8, PI / 2, -PI / 2, 5 * PI / 8]  # TODO: generate programmatically
...
def get_disabled_collisions(pr2):
    ...
    return {(link_mapping[name1], link_mapping[name2]) for name1, name2 in disabled_names if (name1 in link_mapping) and (name2 in link_mapping)}
...
def get_top_grasps(body, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(), max_width=MAX_GRASP_WIDTH, grasp_length=GRASP_LENGTH):
    ...
    if w <= max_width:
        for i in range(1 + under):
            ...
            grasps += [multiply(tool_pose, translate_z, rotate_z, reflect_z, translate_center, body_pose)]
    if l <= max_width:
        for i in range(1 + under):
    ...
    for j in range(1 + under):
        ...
        if w <= max_width:
            ...
            for i in range(2):
                ...
                grasps += [multiply(tool_pose, translate_z, rotate_z, swap_xz, translate_center, body_pose)]  # , np.array([w])
        if l <= max_width:
            translate_z = Pose(point=[x_offset, 0, w / 2 - grasp_length])
            for i in range(2):
                rotate_z = Pose(euler=[i * math.pi, 0, 0])
                grasps += [multiply(tool_pose, translate_z, rotate_z, swap_xz, translate_center, body_pose)]  # , np.array([l])
    return grasps
...
def get_top_cylinder_grasps(body, tool_pose=TOOL_POSE, body_pose=unit_pose(), max_width=MAX_GRASP_WIDTH, grasp_length=GRASP_LENGTH):
    ...
    while True:
        ...
        yield multiply(tool_pose, translate_z, rotate_z, reflect_z, translate_center, body_pose)


def get_side_cylinder_grasps(body, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(), max_width=MAX_GRASP_WIDTH, grasp_length=GRASP_LENGTH, top_offset=SIDE_HEIGHT_OFFSET):
    center, (diameter, height) = approximate_as_cylinder(body, body_pose=body_pose)
...
def get_edge_cylinder_grasps(body, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(), grasp_length=GRASP_LENGTH):
    center, (diameter, height) = approximate_as_cylinder(body, body_pose=body_pose)
    ...
    while True:
        ...
        for i in range(1 + under):
            ...
            yield multiply(tool_pose, rotate_under, translate_yz, rotate_z, reflect_y, translate_center, body_pose)
...
def get_cylinder_push(body, theta, under=False, body_quat=unit_quat(), tilt=0.0, base_offset=0.02, side_offset=0.03):
    ...
    for i in range(1 + under):
        ...
        grasps.append(multiply(tilt_gripper, translate_x, translate_z, rotate_x, rotate_z, reflect_z, translate_center, body_pose))
    return grasps
...
def get_pr2_field_of_view(**kwargs):
    ...
    horizontal = angle_between([view_lower[0], 0, z], [view_upper[0], 0, z])  # 0.7853966439794928
    vertical = angle_between([0, view_lower[1], z], [0, view_upper[1], z])  # 0.6024511557247721
...
def is_visible_aabb(aabb, **kwargs):
    ...
    return not (np.any(body_lower[:2] < view_lower[:2]) or np.any(view_upper[:2] < body_upper[:2]))
...
def get_viewcone(depth=MAX_VISUAL_DISTANCE, camera_matrix=None, **kwargs):
    mesh = cone_mesh_from_support(get_viewcone_base(depth=depth, camera_matrix=camera_matrix))
    assert mesh is not None
    return create_mesh(mesh, **kwargs)


def attach_viewcone(robot, head_name=HEAD_LINK_NAME, depth=MAX_VISUAL_DISTANCE, camera_matrix=None, color=(1, 0, 0), **kwargs):
...
def draw_viewcone(pose=Pose(), depth=MAX_VISUAL_DISTANCE, camera_matrix=None, color=(1, 0, 0), **kwargs):
...
def inverse_visibility(pr2, point, head_name=HEAD_LINK_NAME, head_joints=None, max_iterations=100, step_size=0.5, tolerance=np.pi * 1e-2, verbose=False):
    ...
    with LockRenderer(lock=True):
        with ConfSaver(pr2):
            for iteration in range(max_iterations):
                ...
                correction_conf = np.array([np.dot(angular[mj], correction_euler) for mj in movable_from_joints(pr2, head_joints)])
                if verbose:
                    print("Iteration: {} | Error: {:.3f} | Correction: {}".format(iteration, error_angle, correction_conf))
...
def plan_pause_scan_path(pr2, tilt=0):
    ...
    epsilon = 1e-3
    return [np.array([pan, tilt]) for pan in np.linspace(lower_limit + epsilon, upper_limit - epsilon, n, endpoint=True)]
...
def get_detections(pr2, p_false_neg=0, camera_link=HEAD_LINK_NAME, exclude_links=set(), color=None, **kwargs):
    ...
    for body in get_bodies():
        ...
        if not any(pairwise_collision(cone, obst) for obst in set(get_bodies()) - {pr2, body, cone}) and not any(link_pairs_collision(pr2, [link], cone) for link in set(get_all_links(pr2)) - exclude_links):
...
def visible_base_generator(robot, target_point, base_range=(1.0, 1.0), theta_range=(0.0, 0.0)):
    while True:
